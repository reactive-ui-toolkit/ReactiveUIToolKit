...
namespace Ruitk.SourceGenerator
{
    ...
    public sealed class UitkxGenerator : IIncrementalGenerator
    {
        ...
        private static readonly DiagnosticDescriptor s_generatorAlive = new DiagnosticDescriptor(
            id: "UITKX0000",
            title: "Ruitk.SourceGenerator is active",
            messageFormat: "Ruitk.SourceGenerator loaded for assembly '{0}'. "
                + "Found {1} .uitkx file(s) via disk scan.",
            category: "Ruitk.SourceGenerator",
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterPostInitializationOutput(static ctx =>
            {
                ctx.AddSource(
                    "UITKX_Loaded.g.cs",
                    "// <auto-generated/>\n"
                        + "// Ruitk.SourceGenerator is loaded and running.\n"
                );
            ...
            ImmutableArray<PeerExportsInfo> peerExports = default)
        {
            ...
            foreach (var (file, imports) in importsByFile)
            {
                ...
                string? projRoot = Ruitk.Language.AssetPathUtil.GetProjectRoot(file);
                string root = projRoot != null
                    ? NormPath(projRoot + "/" + Ruitk.Language.UitkxConfig.LoadRoot(dir))
                    : dir;

                foreach (var (names, specifier) in imports)
                {
                    string? tgt = Ruitk.Language.ImportResolver.MapSpecifierToPath(dir, specifier, root, out _);
                    if (tgt == null)
                        continue;
            ...
            var cycle = Ruitk.Language.UitkxImportGraph.FindCycle(edges);
            if (cycle != null)
            {
