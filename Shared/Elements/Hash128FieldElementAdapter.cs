...
{
    public sealed class Hash128FieldElementAdapter : BaseElementAdapter
    {
        public override VisualElement Create() => new Hash128Field();
        ...
        {
            if (element is not Hash128Field field || properties == null)
            {
                PropsApplier.Apply(element, properties);
                return;
            }
            TryApplyProp<Hash128>(properties, "value", v => field.value = v);
        ...
        {
            if (element is not Hash128Field field)
            {
            ...
            next ??= s_emptyProps;
            TryDiffProp<Hash128>(previous, next, "value", v => field.value = v);
            ApplySlotsDiff(field, previous, next);
            PropsApplier.ApplyDiff(element, previous, next);
        }

        private static void ApplySlots(
            BaseField<Hash128> field,
            IReadOnlyDictionary<string, object> properties
        )
        {
            if (properties == null)
        ...
        {
            if (element is Hash128Field field && props is Hash128FieldProps fp)
            {
        ...
        {
            if (
                element is Hash128Field field
                && prev is Hash128FieldProps fp
                && next is Hash128FieldProps fn
            )
            {
