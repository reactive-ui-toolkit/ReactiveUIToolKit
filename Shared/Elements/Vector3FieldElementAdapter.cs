...
{
    public sealed class Vector3FieldElementAdapter : BaseElementAdapter
    {
        public override VisualElement Create() => new Vector3Field();
        ...
        {
            if (element is not Vector3Field field || properties == null)
            {
                PropsApplier.Apply(element, properties);
                return;
            }
            TryApplyProp<Vector3>(properties, "value", v => field.value = v);
        ...
        {
            if (element is not Vector3Field field)
            {
            ...
            next ??= s_emptyProps;
            TryDiffProp<Vector3>(previous, next, "value", v => field.value = v);
        ...
        {
            if (element is Vector3Field field && props is Vector3FieldProps fp)
            {
        ...
        {
            if (
                element is Vector3Field field
                && prev is Vector3FieldProps fp
                && next is Vector3FieldProps fn
            )
            {
