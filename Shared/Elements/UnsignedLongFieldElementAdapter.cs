...
{
    public sealed class UnsignedLongFieldElementAdapter : BaseElementAdapter
    {
        public override VisualElement Create() => new UnsignedLongField();
        ...
        {
            if (element is not UnsignedLongField field || properties == null)
            {
                PropsApplier.Apply(element, properties);
                return;
            }
            TryApplyProp<ulong>(properties, "value", v => field.value = v);
        ...
        {
            if (element is not UnsignedLongField field)
            {
            ...
            next ??= s_emptyProps;
            TryDiffProp<ulong>(previous, next, "value", v => field.value = v);
            ApplySlotsDiff(field, previous, next);
            PropsApplier.ApplyDiff(element, previous, next);
        }

        private static void ApplySlots(
            UnsignedLongField field,
            IReadOnlyDictionary<string, object> properties
        )
        {
            if (properties == null)
        ...
        {
            if (element is UnsignedLongField field && props is UnsignedLongFieldProps fp)
            {
        ...
        {
            if (
                element is UnsignedLongField field
                && prev is UnsignedLongFieldProps fp
                && next is UnsignedLongFieldProps fn
            )
            {
