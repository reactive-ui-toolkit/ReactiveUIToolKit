...
{
    public sealed class RectFieldElementAdapter : BaseElementAdapter
    {
        public override VisualElement Create() => new RectField();
        ...
        {
            if (element is not RectField field || properties == null)
            {
                PropsApplier.Apply(element, properties);
                return;
            }
            TryApplyProp<Rect>(properties, "value", v => field.value = v);
        ...
        {
            if (element is not RectField field)
            {
            ...
            next ??= s_emptyProps;
            TryDiffProp<Rect>(previous, next, "value", v => field.value = v);
            ApplySlotsDiff(field, previous, next);
            PropsApplier.ApplyDiff(element, previous, next);
        }

        private static void ApplySlots(
            BaseField<Rect> field,
            IReadOnlyDictionary<string, object> properties
        )
        {
            if (properties == null)
