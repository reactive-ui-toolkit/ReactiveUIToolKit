...
{
    public sealed class RectIntFieldElementAdapter : BaseElementAdapter
    {
        public override VisualElement Create() => new RectIntField();
        ...
        {
            if (element is not RectIntField field || properties == null)
            {
                PropsApplier.Apply(element, properties);
                return;
            }
            TryApplyProp<RectInt>(properties, "value", v => field.value = v);
        ...
        {
            if (element is not RectIntField field)
            {
            ...
            next ??= s_emptyProps;
            TryDiffProp<RectInt>(previous, next, "value", v => field.value = v);
            ApplySlotsDiff(field, previous, next);
            PropsApplier.ApplyDiff(element, previous, next);
        }

        private static void ApplySlots(
            BaseField<RectInt> field,
            IReadOnlyDictionary<string, object> properties
        )
        {
            if (properties == null)
        ...
        {
            if (element is RectIntField field && props is RectIntFieldProps fp)
            {
        ...
        {
            if (
                element is RectIntField field
                && prev is RectIntFieldProps fp
                && next is RectIntFieldProps fn
            )
            {
