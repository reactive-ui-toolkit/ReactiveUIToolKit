...
{
    public sealed class UnsignedIntegerFieldElementAdapter : BaseElementAdapter
    {
        public override VisualElement Create() => new UnsignedIntegerField();
        ...
        {
            if (element is not UnsignedIntegerField field || properties == null)
            {
                PropsApplier.Apply(element, properties);
                return;
            }
            TryApplyProp<uint>(properties, "value", v => field.value = v);
        ...
        {
            if (element is not UnsignedIntegerField field)
            {
            ...
            next ??= s_emptyProps;
            TryDiffProp<uint>(previous, next, "value", v => field.value = v);
            ApplySlotsDiff(field, previous, next);
            PropsApplier.ApplyDiff(element, previous, next);
        }

        private static void ApplySlots(
            UnsignedIntegerField field,
            IReadOnlyDictionary<string, object> properties
        )
        {
            if (properties == null)
        ...
        {
            if (element is UnsignedIntegerField field && props is UnsignedIntegerFieldProps fp)
            {
        ...
        {
            if (
                element is UnsignedIntegerField field
                && prev is UnsignedIntegerFieldProps fp
                && next is UnsignedIntegerFieldProps fn
            )
            {
