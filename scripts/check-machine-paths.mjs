...
const EXEMPT = [
	...
	// changelog.json, and `changelog.mjs verify` re-emits them; a fix would have to edit frozen bodies.
	{ re: /^ide-extensions~\/(vscode|visual-studio)\/(CHANGELOG|README)\.md$/, why: 'generated from the frozen changelog.json' },
	{ re: /^ide-extensions~\/visual-studio\/UitkxVsix\/overview\.md$/, why: 'generated from the frozen changelog.json' },
	// TEST TREES — synthetic "<drive>:/proj/Assets/UI" fixtures are the subject under test, not a leak.
	// SourceGenerator~/Tests and ide-extensions~/lsp-server/Tests together hold ~80 lines of
	// deliberate Windows-absolute fixtures asserting uitkx namespace derivation and URI handling.
	...
	// SAMPLE ROUTER APP — the `/home/<route>`-style strings under this tree are Router route URLs
	// (React-Router-style path strings), which HOME_ABS cannot distinguish from a Linux home dir.
	// Scoped to UI/ so the sample's own docs (Samples/UIs/PrettyUi/README.md) stay covered.
	{ re: /^Samples\/UIs\/PrettyUi\/UI\//, why: 'sample app: /home/* are Router route URLs, not filesystem paths' },
];
