# Remaining Work â€” consolidated backlog

> Single source of truth for every outstanding item extracted from the archived plans
> (see `Plans~/archive/`). Assembled 2026-07-15 by verifying each plan against the code â€”
...
> `DISCORD_CHANGELOG.md` (operational log), `family-corpus.hash` (**load-bearing** â€” CI drift
...
|---|---|---|---|
| U4 | Multi-root counting mismatch: `DiagnosticsAnalyzer` and `StructureValidator` count render roots with two separate implementations that can disagree â€” extract one shared root counter | two implementations remain (language-lib `DiagnosticsAnalyzer` vs SG `StructureValidator`) | PARITY U4 |
| P-2 | Format-on-save silently no-ops in some VS Code sessions â€” needs a live repro before any change (investigation item) | no repro recorded | V1 P-2 |
...
| O-03 | `GC.Collect(2, Optimized)` per HMR compile â€” measure a 10-save burst; gate it if it stutters | `Editor/HMR/UitkxHmrCompiler.cs` (~2498, ~2783) unconditional | AUDIT-OPT O-03 |
| O-04 / D-HMR-B8 | Shared-`.uss` edit fans out N separate Roslyn compiles (one per dependent) â€” batch into one union compile (defer until a real complaint; one-per-tick drain already landed) | no dependent batching in `UitkxHmrController` | AUDIT-OPT O-04, V1 D-HMR-B8 |
...
| D-HMR-B9 | `TryResolveMissingDependencies` lacks a visited-set/cycle guard (the visited-set in the import fan-out path is a different mechanism) | `Editor/HMR/UitkxHmrController.cs` (~1156â€“1181) | V1 D-HMR-B9 |
...
| HMR-MC | Multi-component files hot-swap only their FIRST component (HMR reads the singular `ComponentName`; `ComponentDeclarations` is never read in `Editor/HMR/`) â€” documented as a Known Issue; full support = per-declaration compile/swap | `Editor/HMR/UitkxHmrCompiler.cs` `Compile()` singular read | triage 2026-07-15 |
| HMR-FSW | If the member-file silence recurs WITH the 2026-07-18 trail in place (save produces neither an `[HMR] Save:` line nor, with Verbose watcher trace on, an `[HMR][trace] FSW` line), the drop is OS-level FSW non-delivery (Mono 8 KB buffer / AV hook) â€” next step is a bounded mtime-sweep fallback over the known `.uitkx` set in the watcher pump (the AssetPostprocessor net cannot help mid-session: `DisallowAutoRefresh` starves it). Trigger to revisit: one field report with the trail present | `Editor/HMR/UitkxHmrFileWatcher.cs` pump; fix wave `fix/hmr-field-wave` | field triage 2026-07-18 |

## 3. Performance

| ID | Item | Evidence / anchor | Source |
|---|---|---|---|
| U-16 | LSP workspace scan walks `Library/`/`Temp/`/`obj/` with no exclusions (only `~` dirs skipped) â€” minutes-long initial scan on big projects | `ide-extensions~/lsp-server/WorkspaceIndex.cs` (~616â€“645) `EnumerateFiles(root, "*.cs", AllDirectories)` | AUDIT-OPT U-16 |
...
| O-05 | Formatter allocation profile â€” span-based line walking (opportunistic; its correctness gates all landed) | `AstFormatter` split/substring-heavy | AUDIT-OPT O-05 |

## 4. Diagnostics

| ID | Item | Evidence / anchor | Source |
|---|---|---|---|
| U1 | Delete/retire never-emitted diagnostic constants + descriptors and add a reflection test enforcing "every registered code has an emission site" (0005/0012/0101/0102/0110 partially retired already â€” 0005/0103 SG descriptors deleted, 0101/0102/0103 marked RETIRED; finish the sweep + the guard test) | `DiagnosticCodes.cs` still lists codes with no emitter | PARITY U1 |
...
| U3 | Diagnostics dropped at the Unity bridge: `Location.None` bridging semantics, `#error` strips the code prefix, discarded `jsxDiags`, unused 0025/0026 descriptors (partially improved by SurfaceLocationlessDiagnostics â€” audit the remainder) | `SourceGenerator~/UitkxPipeline.cs` | PARITY U3 |
...
| ID | Item | Evidence / anchor | Source |
|---|---|---|---|
| CR-T | Custom-rendering test matrix T1â€“T10 (SG emit shape, pool reset, diff/remove, `RedrawKey` `_hasEvents` gating, HMR parity marker) never landed as named tests | `grep OnGenerateVisualContent SourceGenerator~/Tests` â†’ no matches | CUSTOM_RENDERING |
| RT-1 | Clean-clone â†’ first-build validation (was flagged Blocker; never done â€” a CI job or a recorded manual run) | no CI job or record | V1 RT-1 |
| RT-2 | macOS sample validation (deferred â€” make the deferral an explicit recorded decision) | no macOS environment | V1 RT-2 |
...
| ID | Item | Evidence / anchor | Source |
|---|---|---|---|
| P-1a..e | TextMate layer-1 gaps: fragments `<>`, nested generics, verbatim/interpolated/raw strings, char literals, `?.` / `when` | `ide-extensions~/vscode/syntaxes/uitkx.tmLanguage.json` â€” no fragment/verbatim-string rules | V1 P-1 |
| LSP-P1 | Roslyn-formatter pass for `@code`/setup blocks (self-listed polish) | ARCHITECTURE doc Â§polish | LSP-ARCH |
| LSP-P2 | Semantic-token cross-IDE portability audit (self-listed polish) | ARCHITECTURE doc Â§polish | LSP-ARCH |
| LSP-P3 | TmLanguage minimal-fallback cleanup (overlaps P-1) | ARCHITECTURE doc Â§polish | LSP-ARCH |
...
| VP-5 | `VERSIONING_PROCESS.md` still says "update TECH_DEBT.md" â€” retarget the pointer to this file | `VERSIONING_PROCESS.md` (~705) | VERSIONING VP-5 |
| ATLAS | `codebase-index.json` / `repository-atlas.md` are stale (May 3) â€” regenerate or mark generation date prominently | file dates | triage |
...
| Store-1 | Asset Store: create package draft + first upload from a 6.2 editor + Submit (**paused by owner**; price switched to ~$5 â†’ payout/tax setup required first) | no submission occurred | ASSET_STORE |
...
| UG-8 | uGUI sample gallery scenes under `Samples/` + store omit-list review (`Samples/UguiDemo` counter ships) | minimal demo only | pre-store-submission pass |
